...
function UpdateProduct(props) {
    const [pid,setPid]=useState(props.prod.id);
    const [pname,setPname]=useState(props.prod.name);
    const [pdes,setPdec]=useState(props.prod.description);
    const [pprice,setPprice]=useState(props.prod.price);
    ...
    axios.put("https://localhost:7071/api/Product/UpdateProduct",product)
    .then((response) => {
      console.log(response);
      alert("Product Details=>"+pid+","+pname+","+pdes+","+pprice+" updated Successfully...");
  ...
  return (
    <div className="container" style={{textAlign: "center"}}><h2>Update Product Here...</h2>
    <form>
